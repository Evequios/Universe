...
import 'package:wwe_universe/pages/Teams/AddEditTeamsPage.dart';
...
class _TeamsDetailPage extends State<TeamsDetailPage> {
  ...
  Future refreshTeam() async {
    ...
    superstarList = await DatabaseService.instance.readAllSuperstars();
    team = await DatabaseService.instance.readTeam(widget.teamId);
    m1 = await DatabaseService.instance.readSuperstar(team.member1);
    m2 = await DatabaseService.instance.readSuperstar(team.member2);
    if(team.member3 != 0) m3 = await DatabaseService.instance.readSuperstar(team.member3);
    if(team.member4 != 0) m4 = await DatabaseService.instance.readSuperstar(team.member4);
    if(team.member5 != 0) m5 = await DatabaseService.instance.readSuperstar(team.member5);
  ...
  showAlertDialog(BuildContext context) {
    ...
    Widget continueButton = TextButton(
      ...
      onPressed:  () async {
        await DatabaseService.instance.deleteTeam(widget.teamId);
        if(context.mounted){
